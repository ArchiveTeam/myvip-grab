...
LUA_SHA1 = get_hash(os.path.join(CWD, 'myvip.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "myvip-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("myvip-user: %(item_name)s"),
        ]
        ...
        assert item_type in ('100users')
        
        suffixes = string.digits

        if item_type == '100users':
            for url in ['http://myvip.com/profile.php?uid={0}{1}{2}'.format(item_value, a, b) for a in suffixes for b in suffixes]:
        ...
        mypassfield = re.search(r'name="passfield" value="([^"]+)"', myviplogin.text).group(1)
        ...
        myseccode = re.search(r"name='seccode' value='([^']+)'", myviplogin.text).group(1)
        os.system("wget --save-cookies cookies.txt --keep-session-cookies --post-data 'mode=0&screenwidth=1680&flashversion=20.0.0&passfield="+mypassfield+"&email="+myemail+"&"+mypassfield+"="+mypassword+"&redir_if_error=&choise_facebook=0&popup_login=0&t=&url_ending=&seccode="+myseccode+"' --referer http://myvip.com/index.php http://myvip.com/index.php?act=login")
        os.remove('index.php?act=login')
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="myvip"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
